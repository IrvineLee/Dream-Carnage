...
public class EnemyMovement : MonoBehaviour 
{
    ...
    public class Movement
    {
        ...
        public class Direction
        {
            ...
            public Direction(Vector3 direction, float speed, float distance, float startDelay)
            {
                this.direction = direction;
                this.speed = speed;
...
//    public float moveSpeed = 1;
    ...
    int mCurrDirIndex, mCurrActionNum;
	...
	void Update () 
    {
        ...
        else if (movementList.Count > 0)
        {
            ...
            else if (movement.type == Movement.Type.DIRECTION)
            {
                if (mCurrDirIndex > movement.directionList.Count - 1) return;

                Movement.Direction currDir = movement.directionList[mCurrDirIndex];
                currDir.startDelay -= Time.deltaTime;

                if (currDir.startDelay <= 0)
                {
                    transform.position += currDir.direction * Time.deltaTime * currDir.speed;

                    float sqrMagnitude = (mPrevPos - transform.position).sqrMagnitude;
                    if (sqrMagnitude >= currDir.distance)
                    {
                        mCurrDirIndex++;
                        mPrevPos = transform.position;
                    }
    ...
    public void SetMovement(Transform spawnPos, Movement movement)
    {
        ...
        else if (movement.type == Movement.Type.DIRECTION)
        {
            ...
            currMovement.type = Movement.Type.DIRECTION;
            for (int i = 0; i < movement.directionList.Count; i++)
            {
                currMovement.directionList.Add(new Movement.Direction());
                currMovement.directionList[i].direction = movement.directionList[i].direction;
                currMovement.directionList[i].speed = movement.directionList[i].speed;
                currMovement.directionList[i].distance = movement.directionList[i].distance;
                currMovement.directionList[i].startDelay = movement.directionList[i].startDelay;
            }
        }
    }
    ...
    public IEnumerator MoveToWayPoint(int currActionNum)
    {
        ...
        {
            // Break out if the previous movement is not stopped properly.
            if (savedActionNum != mCurrActionNum)
            {
                Debug.Log("BREAK!!!");
            ...
            if (currWayIndex < currMoveThisAct.wayPointList.Count)
            {
                ...
                float delay = currWayPoint.startDelay;
                if (currIndex == currWayIndex && delay != 0)
                {
                    rgBody.velocity = Vector3.zero;
    ...
    public void StopCurrMovement() { rgBody.velocity = Vector3.zero; }
