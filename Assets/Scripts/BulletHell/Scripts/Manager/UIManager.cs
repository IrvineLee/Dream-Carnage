...
public class UIManager : MonoBehaviour 
{
	...
	public Transform player1_UI, player2_UI, player1_PauseUI, player2_PauseUI, gameOver_UI, showScore_UI, showNameInput_UI, gameOverButton_UI, timerUI;

    public float nameInputTimer = 60, nameInputCountDownSpd = 1;
	public int maxPauseButton = 3, maxGameOverButton = 3;
    public Color powerUpMaxColor;
    ...
    class PlayerInfo
    {
        ...
        public bool isScoreCoroutine = false, isMultCoroutine = false, isHighScoreCoroutine = false;
        public int startCurrScore, currScore, toReachScore;
        public int startHighScore, currHighScore = 1, toReachHighScore;
        public float startCurrMult, currMult = 1, toReachMult;
        public float scoreTimer, highScoreTimer, multTimer;
	...
	float mEndScoreTimer, mShowStageTimer;
    bool mIsCoroutine, mIsStartCount, mIsInputName, mIsRedCross, mIsDeactivatedLink, mIsTimerRunOutSfx;
	...
	void Start () 
    {
        ...
        mDefaultLinkColor = mPlayerUIList[0].linkBarImage.color;
		...
		else if (state == State.SHOW_SCORE)
		{
            if (!mIsCoroutine)
            {
                ...
                if (!mIsInputName)
                {
                    ...
                    {
                        AudioManager.sSingleton.StopMainBGM_Sources();
                        AudioManager.sSingleton.StopMainSFX_Sources();
                        AudioManager.sSingleton.PlayCoinCountSfx();
                    }
	...
	public void ShowAutoCollectZone()
	{
		if (isShowAutoCollect) 
		{
            ...
            BGM_Appear.SlideIn("BGM: Aurora Current - Asher Fulero");
		}
	}

    public void ShowBossBGM()
    {
        BGM_Appear.SlideIn("BGM: Significance to Exist - YouFulca");
    }
    ...
    public void EnablePauseScreen(int playerNum)
    {
        ...
        if (AudioManager.sSingleton != null)
        {
            AudioManager.sSingleton.PauseMainBGM_Sources();
            AudioManager.sSingleton.PauseOnSoulRadiusSfx();
            AudioManager.sSingleton.PlayPauseScreenBGM();
        }
    }
    ...
    public void DisablePauseScreen()
    {
        ...
        if (AudioManager.sSingleton != null)
        {
            AudioManager.sSingleton.PlayMainBGM_Sources();
            AudioManager.sSingleton.ResumeOnSoulRadiusSFX();
            AudioManager.sSingleton.StopPauseScreenBGM();
        }
    }

    // Called only when 1 player is playing the game.
    public void GreyOutPlayerUI(int playerID)
    {
        float reduceAmount = 0.38f;
        ColorBrightness(false, mPlayerUIList[playerID - 1].powerLevel_UI.transform, reduceAmount, 1);
        ColorBrightness(false, mPlayerUIList[playerID - 1].highScore_UI.transform, reduceAmount, 1);
        ColorBrightness(false, mPlayerUIList[playerID - 1].score_UI.transform, reduceAmount, 1);
        ColorBrightness(false, mPlayerUIList[playerID - 1].multiplier_UI.transform, reduceAmount, 1);
        ColorBrightness(false, mPlayerUIList[playerID - 1].multX_UI.transform, reduceAmount, 1);

        // Grey out both player's link bar.
        if (!mIsDeactivatedLink)
        {
            ColorBrightness(false, mPlayerUIList[0].linkBarImage.transform, reduceAmount, 1);
            ColorBrightness(false, mPlayerUIList[0].linkBarOutside.transform, reduceAmount, 1);
            ColorBrightness(false, mPlayerUIList[0].percent_UI.transform, reduceAmount, 1);

            ColorBrightness(false, mPlayerUIList[1].linkBarImage.transform, reduceAmount, 1);
            ColorBrightness(false, mPlayerUIList[1].linkBarOutside.transform, reduceAmount, 1);
            ColorBrightness(false, mPlayerUIList[1].percent_UI.transform, reduceAmount, 1);

            // Grey out the other player's link name.
            if (playerID == 1) ColorBrightness(false, mPlayerUIList[1].link_Name.transform, reduceAmount, 1);
            else ColorBrightness(false, mPlayerUIList[0].link_Name.transform, reduceAmount, 1);;
        }

        StartCoroutine(ReduceChildrenColorBrightness(mPlayerUIList[playerID - 1].subjectName, reduceAmount, 1));
        StartCoroutine(ReduceChildrenColorBrightness(mPlayerUIList[playerID - 1].lifePointTrans, reduceAmount, 1));
        StartCoroutine(ReduceChildrenColorBrightness(mPlayerUIList[playerID - 1].bombTrans, reduceAmount, 1));

//            ActivateRedCrossLinkBar();
        mIsDeactivatedLink = true;
    }

    public void ActivateBothLinkBar()
    {
        if (!mIsDeactivatedLink) return;

        float increaseAmount = 0.38f;
        ColorBrightness(true, mPlayerUIList[0].linkBarImage.transform, increaseAmount, 1);
        ColorBrightness(true, mPlayerUIList[0].linkBarOutside.transform, increaseAmount, 1);
        ColorBrightness(true, mPlayerUIList[0].percent_UI.transform, increaseAmount, 1);

        ColorBrightness(true, mPlayerUIList[1].linkBarImage.transform, increaseAmount, 1, mDefaultLinkColor);
        ColorBrightness(true, mPlayerUIList[1].linkBarOutside.transform, increaseAmount, 1);
        ColorBrightness(true, mPlayerUIList[1].percent_UI.transform, increaseAmount, 1);

        ColorBrightness(true, mPlayerUIList[0].link_Name.transform, increaseAmount, 1);
        ColorBrightness(true, mPlayerUIList[1].link_Name.transform, increaseAmount, 1);

        mIsDeactivatedLink = false;
    }
    ...
    public void DeactivateBothLinkBar()
    {
        ...
        ColorBrightness(false, mPlayerUIList[0].link_Name.transform, reduceAmount, 1);
        ColorBrightness(false, mPlayerUIList[1].link_Name.transform, reduceAmount, 1);

        mIsDeactivatedLink = true;
    }

    public bool IsDeactivatedLink { get { return mIsDeactivatedLink; } }
    ...
    public void UpdateLinkBar(int playerNum, float linkVal)
    {
        ...
        {
            string str = "MAX";
            if (mIsDeactivatedLink) str = "STOP";
            currPlayerInfo.percent_UI.text = str;
        }
        else
        {
    ...
    public void StopLinkBar()
    {
        for (int i = 1; i < 3; i++)
        {
            mPlayerUIList[i - 1].percent_UI.text = "STOP";
        }
    }

    public void MakeScoreYellow(int playerNum, bool isDarken)
    {
        mPlayerUIList[playerNum - 1].highScore_UI.color = Color.yellow;
        if (isDarken)
        {
            Color color = mPlayerUIList[playerNum - 1].highScore_UI.color;
            color.r -= 0.38f;
            color.g -= 0.38f;
            color.b -= 0.38f;
            mPlayerUIList[playerNum - 1].highScore_UI.color = color;
        }
    }

    public void MakePowerUpBlue(int playerNum, bool isChangeBlue)
    {
        if (isChangeBlue) mPlayerUIList[playerNum - 1].powerLevel_UI.color = powerUpMaxColor;
        else mPlayerUIList[playerNum - 1].powerLevel_UI.color = Color.white;
    }

    public void UpdateHighScore(int playerNum, int score)
    {
        PlayerInfo currPlayer = mPlayerUIList[playerNum - 1];

        if (currPlayer.toReachHighScore == score) return;

        currPlayer.startHighScore = currPlayer.currHighScore;
        currPlayer.toReachHighScore = score;
        currPlayer.highScoreTimer = 0;
        if (!currPlayer.isHighScoreCoroutine) StartCoroutine(UpdateHighScoreSequence(playerNum));
    }
    ...
    {
        if (timerUI.gameObject.activeSelf) return;
...
//            else if (currChild.name == TagManager.sSingleton.UI_PauseMenuName) mPlayerUIList[index].pauseTrans = currChild;
        ...
        if (index == 0) mPlayerUIList[index].pauseTrans = player1_PauseUI;
        else if (index == 1) mPlayerUIList[index].pauseTrans = player2_PauseUI;
    }
    ...
    void UpdateBossTimer(float duration)
    {
        ...
        if (seconds <= 10)
        {
            ...
            if (!mIsTimerRunOutSfx) StartCoroutine(PlayRunningOutOfTimeSFX(1));
        }
    ...
    void RestartLevel()
    {
        AudioManager.sSingleton.FadeOutPauseBGM ();
    ...
    IEnumerator PlayRunningOutOfTimeSFX(float waitTime)
    {
        mIsTimerRunOutSfx = true;

        if (AudioManager.sSingleton != null) AudioManager.sSingleton.PlayTimerRunningOutSfx();
        yield return new WaitForSeconds(waitTime - 0.05f);

        mIsTimerRunOutSfx = false;
    }
    ...
    IEnumerator AddScoreSequence(int player)
    {
        ...
        while (currPlayer.scoreTimer < scoreCountTime)
		{
            ...
            currPlayer.scoreTimer += Time.deltaTime;
            if (currPlayer.scoreTimer > scoreCountTime) currPlayer.scoreTimer = scoreCountTime;

            float differences = (float)(currPlayer.toReachScore - currPlayer.startCurrScore);
            float currValForCurrTime = (currPlayer.scoreTimer / scoreCountTime) * differences;
    ...
    IEnumerator UpdateHighScoreSequence(int player)
    {
        PlayerInfo currPlayer = mPlayerUIList[player - 1];
        currPlayer.isHighScoreCoroutine = true;

        while (currPlayer.highScoreTimer < scoreCountTime)
        {
            while (state == State.PAUSE)
            {
                yield return null;
            ...
            currPlayer.highScoreTimer += Time.deltaTime;
            if (currPlayer.highScoreTimer > scoreCountTime) currPlayer.highScoreTimer = scoreCountTime;

            float differences = (float)(currPlayer.toReachHighScore - currPlayer.startHighScore);
            float currValForCurrTime = (currPlayer.highScoreTimer / scoreCountTime) * differences;
            currPlayer.currHighScore = (int)Mathf.Ceil((float)currPlayer.startHighScore + currValForCurrTime);

            if (currPlayer.currHighScore > currPlayer.toReachHighScore) currPlayer.currHighScore = currPlayer.toReachHighScore;
            currPlayer.highScore_UI.text = GetScoreWithZero(currPlayer.currHighScore.ToString());
            yield return null;
        }
        currPlayer.isHighScoreCoroutine = false;
    }
    ...
    IEnumerator AnimateMultiplierSequence(int player)
    {
        ...
        while (currPlayer.multTimer < time)
        {
            while (state == State.PAUSE)
            {
                yield return null;
            ...
            currPlayer.multTimer += Time.deltaTime;
            if (currPlayer.multTimer > time) currPlayer.multTimer = time;

            float differences = currPlayer.toReachMult - currPlayer.startCurrMult;
            float currValForCurrTime = (currPlayer.multTimer / time) * differences;
        ...
        multText.text = currPlayer.toReachMult.ToString("F2");
    ...
    void ColorBrightness(bool isIncrease, Transform trans, float plusMinusValue, float speed)
    {
        StartCoroutine(ColorBrightnessIE(isIncrease, trans, plusMinusValue, speed, Color.clear));
    }

    void ColorBrightness(bool isIncrease, Transform trans, float plusMinusValue, float speed, Color defaultColor)
    {
        StartCoroutine(ColorBrightnessIE(isIncrease, trans, plusMinusValue, speed, defaultColor));
    }

    IEnumerator ColorBrightnessIE(bool isIncrease, Transform trans, float plusMinusValue, float speed, Color defaultColor)
    {
        mIsCoroutine = true;
        ...
        while (currMinusVal < plusMinusValue)
        {
            ...
            currMinusVal += val;
            if (currMinusVal > plusMinusValue)
            {
                val = currMinusVal - plusMinusValue;
                currMinusVal = plusMinusValue;
            }

            if (!isIncrease) val = -val;
            ...
            if (defaultColor != Color.clear)
            {
                if (isIncrease)
                {
                    if (color.r > defaultColor.r) color.r = defaultColor.r;
                    if (color.g > defaultColor.g) color.g = defaultColor.g;
                    if (color.b > defaultColor.b) color.b = defaultColor.b;
                }
                else
                {
                    if (color.r < defaultColor.r) color.r = defaultColor.r;
