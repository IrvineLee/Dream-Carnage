...
public class EnvObjManager : MonoBehaviour 
{
	...
	int mDeathPSIndex, mGetHitImpactPSIndex, mGetHitBigImpactPSIndex, mRockDestroyIndex, mRifleIndex, mSniperIndex, mMagnumIndex, mPowerUpTextIndex, mDisappearingBulletIndex;
	...
	List<ParticleSystem> mDisappearingBulletPSList = new List<ParticleSystem>();
    ...
    public ParticleSystem GetDisappearingBulletPS()
    {
        ParticleSystem currPS = mDisappearingBulletPSList[mDisappearingBulletIndex];

        if (mDisappearingBulletIndex + 1 > mDisappearingBulletPSList.Count - 1) mDisappearingBulletIndex = 0;
        else mDisappearingBulletIndex++;
...
//			trans.GetComponent<EnvironmentalObject> ().Initialize ();
    ...
    public void InstantiateAndCache(Transform objTrans, int total)
    {
        ...
        for (int i = 0; i < total; i++) 
        {
			...
			{
                trans.GetComponent<DamageWithinRadius> ().playerID = GameManager.sSingleton.GetMagnumPlayerID;
	...
	public void InstantiateAndCachePS(Transform ps, int total)
	{
		...
		for (int i = 0; i < total; i++) 
		{
            ...
            else if (ps.name == TagManager.sSingleton.UI_DisappearingBulletName) mDisappearingBulletPSList.Add (trans.GetComponent<ParticleSystem>());
		}
	}

    public void TransformBulletIntoScorePU(Vector3 pos, int hitboxIndex)
    {
        ...
        currObj.SetTowardsRandomPlayer(3.0f, hitboxIndex);
		mSmallPowerUpIndex++;
        if (mSmallPowerUpIndex + 1 > mSmallPowerUpList.Count - 1) mSmallPowerUpIndex = 0;
    }
}
