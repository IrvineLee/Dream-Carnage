...
public class MainMenuManager : MonoBehaviour
{
    ...
    public Transform colouredDotL, colouredDotM, colouredDotR;
    ...
    public Transform engOption, engP1Control, engP2Control, engP1EditButton, engP2EditButton, engP1OkButton, engP2OkButton, engButtonLineBg;
    public Transform jpOption, jpP1Control, jpP2Control, jpP1EditButton, jpP2EditButton, jpP1OkButton, jpP2OkButton, jpButtonLineBg;
	...
	public float enterCharSelectWaitTime = 1f;
    ...
    string mJpSelectedCharStr = "決めました", mJpWaitStr = "お待ちください。。", mJpSelectCharStr = "選択中", mJpP2JoinStr = "スタートボタンを押して。。";
    ...
    Vector3 mFrontalVid1Pos, mFrontalVid2Pos, mBackVid1Pos, mBackVid2Pos, mDefaultNamePos, mDefaultBioPos;
    ...
    void Start()
    {
        ...
        mDefaultBioPos = char1Bio.transform.position;
...
//        mLevelController.blackBg.transform.parent.gameObject.SetActive(isEnable);
    ...
    public void SetToDefaultScene()
    {
        ...
        SetText_Alpha(char1Bio, 0);
        SetText_Alpha(char2Bio, 0);
        SetText_Alpha(char3Bio, 0);

        SetText_Alpha(char1NameJp, 0);
        SetText_Alpha(char2NameJp, 0);
        SetText_Alpha(char3NameJp, 0);

        SetText_Alpha(char1BioJp, 0);
        SetText_Alpha(char2BioJp, 0);
        SetText_Alpha(char3BioJp, 0);
    ...
    void StartGame()
    {
        ...
        StartCoroutine(FadeAllChildImageOutIn(characterPotraitList[0], charAlphaChangeSpeed, 0, false, true, () => { }, false));
        ...
        if (language == Language.ENGLISH)
        {
            ...
            StartCoroutine(FadeHorizontalSlideIn(char1Bio.transform, fadeSpeed, moveSpeed, -0.5f));

            engButtonLineBg.gameObject.SetActive(false);
        }
        else
        {
            StartCoroutine(FadeHorizontalSlideIn(char1NameJp.transform, fadeSpeed, moveSpeed, -0.5f));
            StartCoroutine(FadeHorizontalSlideIn(char1BioJp.transform, fadeSpeed, moveSpeed, -0.5f));

            jpButtonLineBg.gameObject.SetActive(false);
        }
    ...
    {
        if ((mIsP1KeybInput && (Input.GetKeyDown(KeyCode.X))) ||
    ...
    void HandleOptionState()
    {
        if (!mIsP1Edit && !mIsP2Edit)
        {
            ...
            {
                if (mOptionIndex == 0 && BGM_Slider.value > 0)
                {
                    OptionSliderControl(AudioManager.sSingleton.bgmSource, ref BGM_Slider, ref BGM_Text, true);
                    AudioManager.sSingleton.bgmSource2.volume = BGM_Slider.value;
                    AudioManager.sSingleton.bgmSource3.volume = BGM_Slider.value;
                    AudioManager.sSingleton.SaveBGMVol(BGM_Slider.value);
                }
                else if (mOptionIndex == 1 && SFX_Slider.value > 0)
                {
                    OptionSliderControl(AudioManager.sSingleton.sfxSource, ref SFX_Slider, ref SFX_Text, true);
                    AudioManager.sSingleton.sfxSource2.volume = SFX_Slider.value;
                    AudioManager.sSingleton.sfxSource3.volume = SFX_Slider.value;
                    AudioManager.sSingleton.sfxSourceLoop.volume = SFX_Slider.value;
                    AudioManager.sSingleton.sfxSourceLoop2.volume = SFX_Slider.value;
                    AudioManager.sSingleton.SaveSFXVol(SFX_Slider.value);
                }
            }
            else if (((mIsP1KeybInput && Input.GetKey(KeyCode.H)) || mHorizontalP1 > 0 || mHorizontalP1Dpad > 0) && sliderDelay <= 0)
            {
                if (mOptionIndex == 0 && BGM_Slider.value < 1)
                {
                    OptionSliderControl(AudioManager.sSingleton.bgmSource, ref BGM_Slider, ref BGM_Text, false);
                    AudioManager.sSingleton.bgmSource2.volume = BGM_Slider.value;
                    AudioManager.sSingleton.bgmSource3.volume = BGM_Slider.value;
                    AudioManager.sSingleton.SaveBGMVol(SFX_Slider.value);
                }
                else if (mOptionIndex == 1 && SFX_Slider.value < 1)
                {
                    OptionSliderControl(AudioManager.sSingleton.sfxSource, ref SFX_Slider, ref SFX_Text, false);
                    AudioManager.sSingleton.sfxSource2.volume = SFX_Slider.value;
                    AudioManager.sSingleton.sfxSource3.volume = SFX_Slider.value;
                    AudioManager.sSingleton.sfxSourceLoop.volume = SFX_Slider.value;
                    AudioManager.sSingleton.sfxSourceLoop2.volume = SFX_Slider.value;
        ...
        else if (mOptionIndex == 3)
        {
            ...
            else if (mIsP1Edit && !mIsP1HorizontalAxisUsed)
            {
                    ...
                    ((mIsP1KeybInput && Input.GetKeyDown(KeyCode.X)) || Input.GetKeyDown(JoystickManager.sSingleton.p1_joystick.backKey)))
                {
                    ...
                    if ((mIsP1KeybInput && Input.GetKeyDown(KeyCode.Z)) || Input.GetKeyDown(JoystickManager.sSingleton.p1_joystick.acceptKey))
                    {
                        if (AudioManager.sSingleton != null) AudioManager.sSingleton.PlayMainMenuAccept3Sfx();
                    }
                    ...
                    if ((mIsP1KeybInput && Input.GetKeyDown(KeyCode.X)) || Input.GetKeyDown(JoystickManager.sSingleton.p1_joystick.backKey))
                    {
                        ...
                        ChangeControlName(mP1SelectedControl, p1Control);
                        if (AudioManager.sSingleton != null) AudioManager.sSingleton.PlayMainMenuBackSfx();
                    }
                }
    ...
    void InitializeLanguage(State state)
    {
        if (language == Language.ENGLISH)
        {
            ...
            if (state == State.HIGH_SCORE || state == State.OPTION || state == State.CREDIT)
                engButtonLineBg.gameObject.SetActive(true);
            ...
            else if (state == State.CHARACTER_SELECT) engCharSelect.gameObject.SetActive(true);
        }
        else if (language == Language.JAPANESE)
        {
            engTrans.gameObject.SetActive(false);
            ...
            if (state == State.HIGH_SCORE || state == State.OPTION || state == State.CREDIT)
                jpButtonLineBg.gameObject.SetActive(true);
            ...
            else if (state == State.CHARACTER_SELECT) jpCharSelect.gameObject.SetActive(true);
        }
    }
    ...
    {
		if (mEnterCharSelectWaitTimer < enterCharSelectWaitTime) 
		{
			mEnterCharSelectWaitTimer += Time.deltaTime;
			return;
		}
        ...
        // Dots.
        EnableDots();
        ColouredDotManager(mCharacterIndex);
        ...
        if (!mIsFading && mMaxPlayer != 2)
        {
            ...
            {
                mFadeCo = FadeTextInOut(p2JpText, p2AlphaChangeSpeed, p2Delay, () => { });
                StartCoroutine(mFadeCo);
            }
        }
        ...
        if (mPlayerNum <= mMaxPlayer)
        {
                ...
                if (mPlayerNum == 1 || mCharacterIndex != mSelectedIndexList[0])
                {
                    ...
                    if (mPlayerNum == 1)
                    {
                        if (mMaxPlayer == 2)
                        {
                            ...
                            if (language == Language.ENGLISH) p1Text.text = mSelectedCharStr;
                            else p1JpText.text = mJpSelectedCharStr;
            ...
            if (!mIsP2Selected && (Input.GetKeyDown(KeyCode.KeypadEnter) || Input.GetKeyDown(JoystickManager.sSingleton.p2_joystick.startKey)))
            {
                ...
                color.a = 1;
                p2Text.color = color;
                p2JpText.color = color;

                if (language == Language.ENGLISH) p2Text.text = mWaitStr;
                else p2JpText.text = mJpWaitStr;

                avatar2SR.gameObject.SetActive(true);
                if (IsAudioManagerOn()) AudioManager.sSingleton.PlayMainMenuAccept2Sfx();
            }
        }
        else
        {
    ...
    void LeftRightCharacterSlect(string dirStr)
    {
        ...
        if (language == Language.ENGLISH)
        {
            if (mCharacterIndex == 0)
            {
                ...
                char2Bio.transform.position = mDefaultBioPos;
                char3Bio.transform.position = mDefaultBioPos;
                SetText_Alpha(char2Bio, 0);
                SetText_Alpha(char3Bio, 0);
            ...
            else if (mCharacterIndex == 1)
            {
                ...
                char1Bio.transform.position = mDefaultBioPos;
                char3Bio.transform.position = mDefaultBioPos;
                SetText_Alpha(char1Bio, 0);
                SetText_Alpha(char3Bio, 0);

                StartCoroutine(FadeHorizontalSlideIn(char2Name.transform, fadeSpeed, moveSpeed, -0.5f));
                StartCoroutine(FadeHorizontalSlideIn(char2Bio.transform, fadeSpeed, moveSpeed, -0.5f));
            }
            else if (mCharacterIndex == 2)
            {
                ...
                StartCoroutine(FadeHorizontalSlideIn(char3Bio.transform, fadeSpeed, moveSpeed, -0.5f));
            }
        }
        else
        {
            if (mCharacterIndex == 0)
            {
                char2NameJp.transform.position = mDefaultNamePos;
                char3NameJp.transform.position = mDefaultNamePos;
                SetText_Alpha(char2NameJp, 0);
                SetText_Alpha(char3NameJp, 0);

                char2BioJp.transform.position = mDefaultBioPos;
                char3BioJp.transform.position = mDefaultBioPos;
                SetText_Alpha(char2BioJp, 0);
                SetText_Alpha(char3BioJp, 0);
            ...
            {
                char1NameJp.transform.position = mDefaultNamePos;
                char3NameJp.transform.position = mDefaultNamePos;
                SetText_Alpha(char1NameJp, 0);
                SetText_Alpha(char3NameJp, 0);

                char1BioJp.transform.position = mDefaultBioPos;
                char3BioJp.transform.position = mDefaultBioPos;
                SetText_Alpha(char1BioJp, 0);
                SetText_Alpha(char3BioJp, 0);

                StartCoroutine(FadeHorizontalSlideIn(char2NameJp.transform, fadeSpeed, moveSpeed, -0.5f));
                StartCoroutine(FadeHorizontalSlideIn(char2BioJp.transform, fadeSpeed, moveSpeed, -0.5f));
            }
            else if (mCharacterIndex == 2)
            {
                char1NameJp.transform.position = mDefaultNamePos;
                ...
                StartCoroutine(FadeHorizontalSlideIn(char3NameJp.transform, fadeSpeed, moveSpeed, -0.5f));
                StartCoroutine(FadeHorizontalSlideIn(char3BioJp.transform, fadeSpeed, moveSpeed, -0.5f));
            }
        }
    }
    ...
    void ResetSelectionForP2()
    {
        ...
        StartCoroutine(FadeAllChildImageOutIn(characterPotraitList[mCharacterIndex], charAlphaChangeSpeed, 0, false, true, () => { }, false));
    ...
    void EnableDots()
    {
        whiteDotL.gameObject.SetActive(true);
        whiteDotM.gameObject.SetActive(true);
        whiteDotR.gameObject.SetActive(true);
    }

    void ColouredDotManager(int charIndex)
    {
        if (charIndex == 0)
        {
            colouredDotM.gameObject.SetActive(false);
            colouredDotR.gameObject.SetActive(false);
            colouredDotL.gameObject.SetActive(true);
        }
        else if (charIndex == 1)
        {
            colouredDotL.gameObject.SetActive(false);
            colouredDotR.gameObject.SetActive(false);
            colouredDotM.gameObject.SetActive(true);
        }
        else if (charIndex == 2)
        {
            colouredDotL.gameObject.SetActive(false);
            colouredDotM.gameObject.SetActive(false);
            colouredDotR.gameObject.SetActive(true);
        }
    ...
    void HandleCredits()
    {
            ...
            Input.GetKeyDown(JoystickManager.sSingleton.p1_joystick.backKey)) 
        {
            ...
            engCredit.gameObject.SetActive(false);
            jpCredit.gameObject.SetActive(false);
            ShowMainMenu ();
        }
    }
    ...
    void OnDisable()
    {
        ...
        jpCharSelect.gameObject.SetActive(false);
    }

    void PlayMoveSfx() { if (IsAudioManagerOn()) AudioManager.sSingleton.PlayMainMenuMoveSfx(); }
    void PlayAcceptSfx() { if (IsAudioManagerOn()) AudioManager.sSingleton.PlayMainMenuAcceptSfx(); }
    void PlayBackSfx() { if (IsAudioManagerOn()) AudioManager.sSingleton.PlayMainMenuBackSfx(); }
    void PlayStartSfx() { if (IsAudioManagerOn()) AudioManager.sSingleton.PlayMainStartGameSfx(); }

    bool IsAudioManagerOn()
    {
        if (AudioManager.sSingleton != null) return true;
        return false;
    }
}
