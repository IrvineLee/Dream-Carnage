...
public class ReviveController : MonoBehaviour
{
    ...
    int mPlayerID, mCharID, mCurrPressNum, mMaxPressNum;
    ...
    void Start()
    {
        ...
        mCharID = mPlayerController.characterID;
    ...
    void Update()
    {
        if (UIManager.sSingleton.state != UIManager.State.NONE || (mPlayerController.state == PlayerController.State.SOUL || 
            (mOtherPlayerController != null && mOtherPlayerController.state != PlayerController.State.SOUL))) return;
        ...
        if (mIsInsideCircle)
        {
               ...
               (mPlayerID == 2 && (Input.GetKeyDown(KeyCode.Semicolon) || Input.GetKeyDown(mPlayerController.GetJoystickInput.reviveKey))))
            {
                ...
                if (mCurrPressNum >= mMaxPressNum)
                {
                    ...
                    {
                        if (mCharID == 1) AudioManager.sSingleton.PlayC1_GiveReviveAudio();
                        else if (mPlayerID == 2) AudioManager.sSingleton.PlayC2_GiveReviveAudio();
                        else if (mPlayerID == 3) AudioManager.sSingleton.PlayC3_GiveReviveAudio();
                    }
                }
            }
        }
...
//        mIsRevived = true;
        mCurrPressNum = 0;
		if (mOtherFillUpImage != null) mOtherFillUpImage.fillAmount = 0;
		if (mOtherFillUpImage != null) otherWithinCirclePS.Stop();
    }
    ...
    {
        if (other.tag == TagManager.sSingleton.reviveCircleTag && mOtherPlayerController.state == PlayerController.State.SOUL)
        {
            // Stop timer.
            mOtherPlayerSoul.StopTimer();
            mIsInsideCircle = true;
            if (AudioManager.sSingleton != null) AudioManager.sSingleton.PlayOnSoulRadiusSfx();
            Debug.Log("Step In");
        }
    }
    ...
    {
//        if (mPlayerController.state == PlayerController.State.SOUL) return;
//
//        if (other.tag == TagManager.sSingleton.reviveCircleTag && mOtherPlayerController.state == PlayerController.State.SOUL)
//        {
//            mOtherPlayerSoul.StopTimer();
...
//        }
    ...
    void OnTriggerExit2D(Collider2D other)
    {
        if (other.tag == TagManager.sSingleton.reviveCircleTag)
        {
            ...
            // Start timer.
            mOtherPlayerSoul.StartTimer();
//            mIsRevived = false;
            mIsInsideCircle = false;
            otherWithinCirclePS.Stop();
            if (AudioManager.sSingleton != null) AudioManager.sSingleton.StopOnSoulRadiusSfx();
            Debug.Log("Step Out");
        }
    }
}
