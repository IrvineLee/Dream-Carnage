...
[CustomEditor(typeof(EnemyBulletControl))]
public class EnemyBulletControlCI : Editor 
{
    EnemyBulletControl mSelf;

    List<bool> mIsShowBulletList = new List<bool>();

    void OnEnable () 
    {
        mSelf = (EnemyBulletControl)target;
        Initialize();
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        {
            mSelf.AddBulletToList();
            mIsShowBulletList.Add(true);
        }
        else if (GUILayout.Button("Del", GUILayout.Width(40)))
        {
            mSelf.DelBulletToList();
            mIsShowBulletList.RemoveAt(mIsShowBulletList.Count - 1);
        }
