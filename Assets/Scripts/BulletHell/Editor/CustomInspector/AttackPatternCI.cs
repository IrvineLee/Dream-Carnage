...
public class AttackPatternCI : Editor 
{
	...
	public override void OnInspectorGUI()
	{
        ...
        else if (mSelf.ownerType != AttackPattern.OwnerType.PLAYER)
        {
            ...
            EditorGUILayout.PropertyField(serializedObject.FindProperty("attackNum")); 
            EditorGUILayout.PropertyField(serializedObject.FindProperty("isIncreaseAtkNum")); 

            if (mSelf.isIncreaseAtkNum) EditorGUILayout.PropertyField(serializedObject.FindProperty("numOfAtkBefIncrease")); 
        }
        ...
        if (BulletManager.sSingleton != null)
        {
            ...
            {
                SpriteRenderer sr = currBulletData.enemyBulletTransList[mSelf.mainBulletIndex].GetComponent<SpriteRenderer>();
                if (sr != null) mainTexture = sr.sprite.texture;
                else mainTexture = currBulletData.enemyBulletTransList[mSelf.mainBulletIndex].GetComponentInChildren<SpriteRenderer>().sprite.texture;
            }
        ...
        else if (mSelf.template == AttackPattern.Template.SHOOT_AROUND_IN_CIRCLE)
        {
            ...
            EditorGUILayout.PropertyField(serializedObject.FindProperty("angleStart"));
            EditorGUILayout.PropertyField(serializedObject.FindProperty("isResetAngAftPause"));

            if (mSelf.isResetAngAftPause)
                EditorGUILayout.PropertyField(serializedObject.FindProperty("angleAfterPause"));

            EditorGUILayout.PropertyField(serializedObject.FindProperty("isOneStartPoint"));
        }
        else if (mSelf.template == AttackPattern.Template.DOUBLE_SINE_WAVE)
        {
            ...
            EditorGUILayout.PropertyField(serializedObject.FindProperty("isGetNewTargetAftPause"));
            EditorGUILayout.PropertyField(serializedObject.FindProperty("isOneStartPoint"));
        }
